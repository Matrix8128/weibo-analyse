...
public class Friendships extends Weibo {
	 ...
	 * 
	 * 4月16号注：返回的实际关注用户或者粉丝用户数量会比显示数量少，因为新浪自动过滤掉了部分广告用户
	 */
	public UserWapper myGetFriendsByID(String id, int count, int cursor)
			throws WeiboException {
		return User.constructWapperUsers(client.get(
				WeiboConfig.getValue("baseURL") + "friendships/friends.json",
				new PostParameter[] { new PostParameter("uid", id),
	 ...
	 * 4月16号注：返回的实际关注用户或者粉丝用户数量会比显示数量少，因为新浪自动过滤掉了部分广告用户
	 */
	public UserWapper myGetFriendsByScreenName(String screen_name, int count,
			int cursor) throws WeiboException {
		return User.constructWapperUsers(client.get(
				WeiboConfig.getValue("baseURL") + "friendships/friends.json",
				new PostParameter[] {
	 ...
	 * @author Edward
	 * @param uid
	 * @return all biFriendsIds
	 * @throws WeiboException
	 */
	public ArrayList myGetFriendsBilateralIds(String uid) throws WeiboException {
		
		int page=1;
		int count=50;
		ArrayList ids=new ArrayList();
		long total_num = ids.size()+1;
		try {
			while(ids.size()<total_num){
				...
				page++;
				JSONObject jsonResult = res.asJSONObject();
				total_num=jsonResult.getLong("total_number");
				JSONArray list = jsonResult.getJSONArray("ids");
