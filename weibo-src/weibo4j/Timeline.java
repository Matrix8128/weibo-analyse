...
public class Timeline extends Weibo {
	 ...
	 * @param uid
	 * @param totalNum
	 *            需要返回的微博数量,count 的倍数，如果不足则返回全部
	 * @return
	 * @throws WeiboException
	 */
	public StatusWapper myGetUserTimelineByUid(String uid, int totalNum) throws WeiboException {
		int count=100;
		int page = 1;
		StatusWapper result = null;
		while (result == null || result.getStatuses().size() < totalNum) {
			StatusWapper temp = Status.constructWapperStatus(client.get(
					WeiboConfig.getValue("baseURL")
							+ "statuses/user_timeline.json",
					new PostParameter[] { new PostParameter("uid", uid),
							new PostParameter("count", count),
			...
			if(result==null){
				...
				for(Status s:temp.getStatuses()){
					result.getStatuses().add(s);
				}
				temp.setStatuses(result.getStatuses());
				result=temp;
			}
			if(result.getTotalNumber()<totalNum){
				totalNum=(int)result.getTotalNumber();
			}
			page++;
		}
		return result;
